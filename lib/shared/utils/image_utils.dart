...
class ImageUtils {
  ...
  /// Returns the image file or null if cancelled
    ...
    try {
      ...
      if (image != null) {
        ...
        if (await file.exists()) {
          ...
          print('? [ImageUtils] File does not exist at path: ${image.path}');
          throw Exception('Captured image file does not exist');
        }
      }
      print('?? [ImageUtils] No image captured by user');
      return null;
    } catch (e, stackTrace) {
      print('? [ImageUtils] Error in pickFromCamera: $e');
  ...
  static Future<File?> pickFromGallery() async {
    try {
      print('??? [ImageUtils] Starting pickFromGallery...');
      final XFile? image = await _picker.pickImage(
      ...
      if (image != null) {
        ...
        if (kIsWeb) {
          ...
          print('? [ImageUtils] Image bytes loaded: ${bytes.length} bytes');
      ...
      print('?? [ImageUtils] No image selected by user');
      return null;
    } catch (e, stackTrace) {
      print('? [ImageUtils] Error in pickFromGallery: $e');
      print('Stack trace: $stackTrace');
      throw Exception('Failed to pick image from gallery: $e');
    }
  }

  /// Pick an image from gallery and return XFile (works better on web)
  /// Returns the XFile or null if cancelled
  static Future<XFile?> pickImageAsXFile() async {
    try {
      print('??? [ImageUtils] Starting pickImageAsXFile...');
      final XFile? image = await _picker.pickImage(
        source: ImageSource.gallery,
        maxWidth: 1920,
        maxHeight: 1080,
      ...
      if (image != null) {
        print('? [ImageUtils] Image picked: ${image.path}');
        print('? [ImageUtils] Image name: ${image.name}');
        return image;
      }
      print('?? [ImageUtils] No image selected by user');
      return null;
    } catch (e, stackTrace) {
      print('? [ImageUtils] Error in pickImageAsXFile: $e');
      print('Stack trace: $stackTrace');
      throw Exception('Failed to pick image: $e');
    }
  }

  /// Convert File to Uint8List for API upload
  static Future<Uint8List> fileToBytes(File file) async {
    if (kIsWeb) {
      // On web, we can't use file.readAsBytes() directly
      throw Exception('Use xFileToBytes for web platform');
    }
    return await file.readAsBytes();
  }

  /// Convert XFile to Uint8List (works on all platforms including web)
